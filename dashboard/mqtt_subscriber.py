...
def _to_naive_utc(dt):
    """Convert a datetime to naive UTC (no tzinfo)."""
    if dt is None:
        return None
...
def on_message(client, userdata, msg):
    try:
        ...
        mmwave_presence = 1 if bool(
            payload.get("mmwave_presence", False)) else 0
        occupancy = 1 if bool(payload.get(
            "occupancy", mmwave_presence == 1 or headcount > 0)) else 0
        now = utcnow_naive()
        raw_event_time = _parse_edge_timestamp(payload.get("event_timestamp"))
        event_time = _to_naive_utc(raw_event_time) or now
        db = get_db()
        if event_id:
            existing = db["sensor_history"].find_one(
        ...
        print(
            f"[MQTT][DEBUG] About to insert into sensor_history: {sensor_doc}")
        db["sensor_history"].insert_one(sensor_doc)
        print(
            f"[MQTT][DEBUG] Inserted into sensor_history for {room_id} event_id={event_id}")
        print(
            f"[MQTT] Processed update for {room_id} headcount={headcount} mmwave={mmwave_presence} occupancy={occupancy}")
...
if __name__ == "__main__":
    ...
    print(
        f"[MQTT] Subscribing to rooms/+/headcount on {MQTT_BROKER}:{MQTT_PORT}")
    client.loop_forever()
